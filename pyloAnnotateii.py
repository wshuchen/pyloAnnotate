...
# pyloAnnotateii.py by Wenshu Chen, 2023
...
# This version uses Heng Li's implicit interval tree (iitree)
# https://github.com/lh3/cgranges).

# Compared with pyloAnnotate.py:
# Much faster for interval conversion, but much slower for points thus
# impractical for large amounts of points.
...
from pyloiitree import LiftOver

def get_liftover_interval(chain_file, query_interval):
    """
    Single interval search. Large interval typically will have a result of
    multiple blocks. This is particularly true with cross-species
    conversion.
    A point is an interval with length == 1.
    """
    lo = LiftOver(chain_file)
    result = []
    unlifted = []
    query = query_interval.split(",")
    chrom, start, end = query[0], int(query[1]), int(query[2])
    result = lo.convert_coordinate(chrom, start, end)
    if result is None:
        print(f"No chain data for query: {chrom}, {start}, {end}")
        sys.exit()
    if result == []:
        print(f"No target for query: {chrom}, {start}, {end}")
        unlifted.append((f"{chrom}\t{start}\t{end}\t+"))
    return result, unlifted

def get_liftover_file(chain_file, interval_file):
    '''
    Convert intervals in a file.
    Also good for a small number of points, but not a big file.
    '''
    lo = LiftOver(chain_file)
    intervals = []
    result = []
        ...
        for line in f:
            if not line.startswith("chr"):
                ...
                if len(line) == 1:
                    ...
                    continue
                intervals.append((line[0], int(line[1]), int(line[2])))
        intervals = sorted(intervals)
    for i in intervals:
        chrom, start, end = i[0], i[1], i[2]
        target = lo.convert_coordinate(chrom, start, end)
        if target is None:
            print(f"No chain data for query: {chrom}, {start}, {end}")
            continue
        if len(target) == 0:
            print(f"No target for query: {chrom}, {start}, {end} ({count})")
            unlifted.append((f"{chrom}\t{start}\t{end}\t+"))
            count += 1
            continue
...
def main():
    ...
    parser.add_argument("-f", "--interval_file", type=str, 
                        help="query intervals in a file in BED format")
    ...
    parser.add_argument("-u", "--unlifted", action="store_true",
                        help="annotate unlifted intervals")
    parser.add_argument("-m", "--merge", action="store_true",  
                        help="merge interval blocks")   
    ...
    # otherwise only do coordinate conversion.
    # Result will be saved as a BED (no annotation)
    # or a tab-delimited file (with annotation).
    # Annotation of unlifted intervals is optional.  
    ...
    if args.interval_file:
        print("\nMapping query coordinates to target")
        print(f"Query file: {args.interval_file}")
        print(f"Chain file: {args.chain_file}\n")
        query_name = os.path.basename(args.interval_file).split(".")[0]
        chain_name = os.path.basename(args.chain_file).split(".")[0]
        out_name = f"{query_name}_{chain_name}"
        result, unlifted = get_liftover_file(chain_file, interval_file)
    ...
    if args.query_gff and args.target_gff:
        ...
        if args.merge:
            ...
            BedTool.from_dataframe(annotation).saveas(out_file)
